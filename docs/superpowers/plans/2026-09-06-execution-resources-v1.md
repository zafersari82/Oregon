...
- [x] **Step 1: Add test consumers before production code.** Create the execution crate manifest and an empty forbid-unsafe lib to register the tests. The consensus test imports the not-yet-existing module. Write table-driven tests for every spec §7 boundary plus invalid configuration and atomic rejection. Representative assertions:
